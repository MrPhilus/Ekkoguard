      ...
      { ({ _field, meta }) => {
        ...
        return (
          <label className="label p-0 pt-1">
            <span
              className={ `flex items-center gap-2 ${!meta.value && "opacity-30"
                }` }
            >
              { !meta.value ? (
                <BsCheckCircleFill />
          ...
          <div className="form-control w-full">
            { label && (
              <label className="label" htmlFor={ props.name }>
                <span className="label-text font-bold text-md xl:text-md capitalize">
                  { label }
                </span>
              </label>
            ) }
            ...
            <div className={ `relative w-full rounded-md ${className}` }>
              <input
                ...
                onInput={ (e) => {
                  if (
                    props.type === "tel" &&
                    (isNaN(e.target.value * 1) || e.target.value.length > 10)
                  )
              ...
              { props.type === "tel" ? (
                <span
                  className={ `absolute font-semibold left-4 top-1/2 -translate-y-1/2` }
                >
                  234
                </span>
              ) : (
            ...
            <label className="label p-0 pt-1">
              <small className="label-text-alt text-error flex items-center min-h-[1rem] self-start">
                { showErrorMessage && meta.touched && meta.error && (
                  <ErrorMessage component={ ErrorWrapper } name={ props.name } />
                ) }
              </small>

              { props.type === "password" && props.passwordreset_path && (
                <Link
                  to={ props.passwordreset_path }
                  type="button"
                  className={ `text-primary xl:text-xl link link-hover 
