...
const OTPVerification = () => {
  ...
  const [
    sendOTP,
    { data: OTPData, error: sendOTPError, isLoading: sendingOTP },
  ] = useSendOTPMutation();
  const [
    verifyOTP,
    {
    ...
    <AuthLayout>
      {verificationError && (
        <p className={`bg-error/50 text-error-content py-2 px-4 rounded-md`}>
          An error occured: ({verificationError?.data?.title}). Please try again
          with a different OTP
        </p>
      )}
      {sendOTPError && (
        <p className={`bg-error/50 text-error-content py-2 px-4 rounded-md`}>
      ...
      {(phoneNumber === "" || sendingOTP) &&
      (!OTPData || OTPData.status !== "OK") ? (
          ...
          {(formik) => (
            <Form>
              ...
              >
                {sendingOTP ? (
                  <span>
                    <span className={`loading loading-bars`} /> Sending OTP
                  </span>
                ) : (
                  `Send OTP`
                )}
        ...
        <>
          <p>{`Enter the four-digit code sent to +${
          ...
          {verifyingOTP ? (
            <div className={`btn bg-olive-500 text-white w-full mt-4`}>
          ...
          <p className="mt-4">
            ...
            <button
              className={` mt-2 font-semibold py-2 ${
                resendTime === 0 ? "link link-hover italic" : "opacity-50"
              }`}
            ...
            >
              Resend{" "}
              {resendTime === 0 || sendOTPError ? "now" : "in " + resendTime}
            </button>
          </p>
        </>
      )}
